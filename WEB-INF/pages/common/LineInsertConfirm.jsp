...
<jsp:include page="/WEB-INF/pages/common/lineHeader.jsp"></jsp:include>
...
function doSubmit(choice){
	...
	if(choice=='next'){
		document.fm.action="LineInsertServlet";
		alert("登録しました");
	}
	else{
		document.fm.action="LineInsertInputServlet";
...
<table>
 ...
 <tr>
 <td><c:out value="${lineDTO.departure_stop_id}"/></td><td><c:out value="${lineDTO.start_time}"/></td>
 </tr>
 <tr>
 <td>到着停留所</td><td>到着時刻</td>
 </tr>
 <tr>
 <td><c:out value="${lineDTO.arrival_stop_id}"/></td><td><c:out value="${lineDTO.end_time}"/></td>
 </tr>
</table>
...
<input type="hidden" name="raw_start_date" value='<c:out value="${raw_start_date }"/>'/>
<input type="hidden" name="raw_end_date" value='<c:out value="${raw_end_date }"/>'/>
